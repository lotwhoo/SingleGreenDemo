...
### M12 Audio / ASR Layering（PR1–PR3 第一段本地完成，2026-09-01）

`VoiceChatCore` Package 内已新增内部 `ASRDomain`、`AudioCaptureApple` 与 `ASRSupervision` Target：依次承载 provider-neutral 契约、Apple 音频实现和跨 Session 的旧事件隔离/有限恢复/类型化降级。提词器 PTT 选择手动降级，对话 PTT 选择可重试失败；生产恢复预算当前保持 0，等待真实故障矩阵后再确认次数与退避。当前本地证据为七 Package 严格并发/WAE **589/589**、架构 **16** 个负向 fixture。Voice Activated Supervisor、锁定工具链 API baseline、App、真机、真实路由故障矩阵和真实 ASR 仍待独立验证，详见 [M10–M17 升级计划](./docs/tasks/2026-08-31-architecture-reliability-update-plan.md)。
